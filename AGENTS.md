...
- Follow the repository Oxfmt configuration (`.oxfmtrc.json`): two spaces, single quotes, trailing
  commas, and no parentheses around a single arrow-function parameter.
- Prefer `type` aliases over `interface`; Oxlint enforces this convention.
...
Run `npx oxfmt --check "./src/**/*.{ts,tsx,js,css}"` (the same scope as format CI) and
